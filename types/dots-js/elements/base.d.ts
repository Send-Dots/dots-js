import { DotsElementType } from '../elements-group';

export type DotsElementBase = {
  /**
 ...
 */
export interface DotsElementStyle {
  /**
   * Base variant—all other variants inherit from these styles.
   */
  base?: DotsElementStyleVariant;

  /**
   * Applied when the element has valid input.
   */
  complete?: DotsElementStyleVariant;

  /**
   * Applied when the element has no customer input.
   */
  empty?: DotsElementStyleVariant;

  /**
   * Applied when the element has invalid input.
   */
  invalid?: DotsElementStyleVariant;
}

/**
 * An object with `CSSProperties` supported by Dots.js.
 * Pseudo-classes and pseudo-elements can also be styled using a nested object inside of a variant.
 */
export interface DotsElementStyleVariant extends DotsElementCSSProperties {
  ':hover'?: DotsElementCSSProperties;

  ':focus'?: DotsElementCSSProperties;

  '::placeholder'?: DotsElementCSSProperties;

  '::selection'?: DotsElementCSSProperties;

  ':-webkit-autofill'?: DotsElementCSSProperties;

  /**
   * Available for all elements except the `paymentRequestButton` element
   */
  ':disabled'?: DotsElementCSSProperties;

  /**
   * Available for the `cardNumber`, `cardExpiry`, and `cardCvc` elements.
   */
  '::-ms-clear'?: DotsElementCSSProperties & { display: string };
}

/**
 * CSS properties supported by Dots.js.
 */
export interface DotsElementCSSProperties {
  /**
...
/**
 * Use `Classes` to set custom class names on the container DOM element when the Dots element is in a particular state.
 */
export interface DotsElementClasses {
  /**
   * The base class applied to the container.
   * Defaults to `DotsElement`.
   */
  base?: string;

  /**
   * The class name to apply when the `Element` is complete.
   * Defaults to `DotsElement--complete`.
   */
  complete?: string;

  /**
   * The class name to apply when the `Element` is empty.
   * Defaults to `DotsElement--empty`.
   */
  empty?: string;

  /**
   * The class name to apply when the `Element` is focused.
   * Defaults to `DotsElement--focus`.
   */
  focus?: string;

  /**
   * The class name to apply when the `Element` is invalid.
   * Defaults to `DotsElement--invalid`.
   */
  invalid?: string;

  /**
   * The class name to apply when the `Element` has its value autofilled by the browser (only on Chrome and Safari).
   * Defaults to `DotsElement--webkit-autofill`.
   */
  webkitAutofill?: string;
}

export interface DotsElementChangeEvent {
  /**
